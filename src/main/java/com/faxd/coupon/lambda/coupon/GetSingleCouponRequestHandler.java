...
public class GetSingleCouponRequestHandler
        implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent>{
    private final Logger logger = LoggerFactory.getLogger(GetSingleCouponRequestHandler.class);
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent request, Context context) {
        ...
        String couponCode = request.getQueryStringParameters().get(CouponTable.SORT_KEY.getName());
        logger.info("Fetching product {} in platform: {} ", couponCode,platform);
        Optional<CouponGetDto> coupon = couponService.getCouponByPlatformAndCouponCode(platform, couponCode, new CouponGetDto());
        if (coupon.isEmpty()) {

            return new APIGatewayProxyResponseEvent()
                    .withStatusCode(404)
                    .withBody("{\"message\": \"Coupon code not found or not available\"}");
        }
        logger.info(coupon.toString());
        try {
            logger.info("get dto: {}",coupon.get());
            return new APIGatewayProxyResponseEvent()
                        .withStatusCode(200)
                        .withBody(objectMapper.writeValueAsString(coupon.get()));
